...
def searchRestaurant(request):
    if request.method == 'POST':
        ...
        #form = AddRestaurant(request.POST, instance=rest)
        ...
        return render(request,'app/modifyRestaurant.html', {'data': context})
        #return render(request,'app/modifyRestaurant.html', {'form': form})
    else:
...
def getRestaurant(request):
    if request.method == 'POST':
        ...
        idRestaurant = request.POST.get('searched_id')
        print(idRestaurant)
    return HttpResponseRedirect('/restaurantes/listRestaurants/')


@login_required
def modifyDatasRestaurant(request):
    if request.method == 'POST':
        context = {}
        nameRestaurant = request.POST.get('nameRestaurant')
        zipcodeRestaurant = request.POST.get('zipcodeRestaurant')
        cityRestaurant = request.POST.get('cityRestaurant')
        streetRestaurant = request.POST.get('streetRestaurant')

        rest = restaurantes.objects(name = nameRestaurant)
        for d in rest:
            latRestaurant = d.address.coord.lat
            lonRestaurant = d.address.coord.lon

        data_dict = {'address':
            {
                'city': cityRestaurant,
                'street': streetRestaurant,
                'zipcode': zipcodeRestaurant,
                'coord': {
                    'lat': latRestaurant,
                    'lon': lonRestaurant
                }
        ...
        restaurantes.objects.filter(name = nameRestaurant).update(**data_dict)
        #rest.updateOne({name: nameRestaurant}, {address.zipcode: zipcodeRestaurant}, { upsert: true })
